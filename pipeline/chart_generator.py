...
def generate_weekly_esr_or_inspections_chart(
    ...
    figure = px.line(
        ...
        title=(
            f"Weekly U.S. {commodity_display} {value_column.replace('_', ' ').title()}<br>"
    ...
    if home:
        json_path = (
            json_dir
             / f"{data_type}_us_{commodity.lower()}_to_{country.lower()}_{value_column}_last_5_years_{file_suffix}_home.json"
        )
    else:
        json_path = (
            json_dir
             / f"{data_type}_us_{commodity.lower()}_to_{country.lower()}_{value_column}_last_5_years_{file_suffix}.json"
        )
...
def generate_weekly_psd_chart(
    ...
    figure = px.bar(
        ...
        title=(
            f"{country.replace("-", " ").title()} {commodity.replace("-", " ").title()} {attribute.replace('_', ' ').title()} "
            f"(as of {latest_date})"
        ),
        labels={
